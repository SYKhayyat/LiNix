...
| **S20** | **Drift for extras is not built (found during S12).** The forward direction is done — `apply_repositories`/`apply_dependents` install every `repo:`/`shim:`/`service:`/`link:` a config declares. But **deleting** one of those lines does nothing: the planner computes drift for *packages* (managed state vs desired `.packages`), and nothing records which extras were applied last time, so a removed `service:nginx` leaves the unit enabled and a removed `link:` leaves the config file in place. The clean shape mirrors package drift: record applied extras in LiNix's own state (beside the registry, II.1 data root — **not** in your files, II.16), diff declared-extras against applied-extras on each `sync`, and reverse the delta — `service` → stop+disable, `link` → remove the managed file (restoring the `.linix-backup` if one exists), `shim` → `remove_shim` (already ownership-safe). Each backend's removal path already exists; the missing piece is the *applied-extras ledger* and the diff. Belongs with the one retention/state engine work — **Phase 4** (locks and state) — because it needs a durable record of what was applied, which is the same machinery `locks/` introduces. **Honest scope of the gap:** without that ledger `sync` cannot even *detect* a removed extra (it has nothing to diff against), so there is no interim warning to add — the whole feature waits on the ledger. Document it in II.16 so users know the teardown is manual until Phase 4, rather than discovering it. → **Phase 4** |
| **S21** | **`schedule:` is entirely unwired, and it has no file-context check (found during S12).** The layout has `schedules_file()` (`config_root/schedules`) but the resolver never reads it: a `schedule:` line only ever lands in `extras`, and `sync` warns it is unapplied. Wiring it is one job with three parts: (1) **read the `schedules` file** in resolution, the same `when`-gated way modules are read; (2) **provision the tasks** through the existing `SchedulerManager`/`TaskProvisioner` (systemd timers / launchd / Task Scheduler — the machinery exists, `schedule` the CLI command already drives it); (3) **the file-context rule** II.2 states — a `schedule:` line belongs in the `schedules` file, and one in a module/profile is an error, not silently parsed. Part (3) cannot be enforced before (1) gives a correct `schedule:` somewhere to live. → **Phase 5** |
