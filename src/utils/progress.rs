...
impl ConsoleProgress {
    pub fn new(enabled: bool) -> Self {
        Self { multi: Arc::new(MultiProgress::new()), enabled }
    }
}
...
impl ProgressReporter for ConsoleProgress {
    ...
    fn println(&self, message: &str) {
        if self.enabled { let _ = self.multi.println(message); } else { println!("{}", message); }
    }
}

struct IndicatifHandle { pb: ProgressBar }
...
pub fn create_progress_reporter(enabled: bool) -> Arc<dyn ProgressReporter> {
    if enabled { Arc::new(ConsoleProgress::new(true)) } else { Arc::new(SilentProgress) }
}