...
impl PackageManager for BunManager {
    ...
    async fn list_installed(&self) -> Result<Vec<Package>> {
        // Bun doesn't have a built-in global list command
        // We'd need to check bun install cache or similar
        Ok(Vec::new())
    }

    async fn upgrade(&self, sudo: bool) -> Result<()> {
        info!("Upgrading all bun packages");
        self.executor.run("bun", &["update"], sudo).await?;
        Ok(())
    }
