...
impl Journal {
    ...
    /// Packages this log says the machine finished installing and has not since finished
    /// removing.
    ///
    /// This is the durable half of a pair that can disagree. Ownership is recorded in memory
    /// as each install completes and serialised to `registry.json` once, at the end of a run
    /// that succeeded; the log is written per operation, before and after each one. A run
    /// killed between an install completing and that final write therefore leaves the package
    /// installed, `Completed` here, and in no registry — and nothing puts it right, because
    /// `heal` only replays what is still open and no later sync reinstalls something already
    /// there.
    ///
    /// **A removal cancels an install, and a tie cancels it too.** A package LiNix installed,
    /// gave up, and that somebody then put back by hand is not LiNix's, and reading only the
    /// install would claim it. The entries are a map keyed by id, so two transitions recorded
    /// in the same second have no order between them — the strict comparison is what keeps
    /// this answer from depending on which one a HashMap happens to yield first, and it errs
    /// towards claiming nothing.
    pub fn completed_installs(&self) -> Vec<PackageSpec> {
        let mut installs: HashMap<(&str, &str), (&PackageSpec, i64)> = HashMap::new();
        let mut removals: HashMap<(&str, &str), i64> = HashMap::new();
        for entry in self.entries.values() {
            if entry.status != ActionStatus::Completed {
                continue;
            }
            // `finished_at_unix` is set on reaching a terminal state and `Completed` is one,
            // so the fallback is unreachable here — and reading the start rather than
            // dropping the entry keeps a hand-edited log from silently losing one.
            let at = entry.finished_at_unix.unwrap_or(entry.started_at_unix);
            let id = entry.action.identity();
            match &entry.action {
                JournalAction::Install(spec) => {
                    installs
                        .entry(id)
                        .and_modify(|held| {
                            if at > held.1 {
                ...
                // A script is not a package and owns nothing.
                JournalAction::Exec { .. } | JournalAction::ExecUndo { .. } => {}
            }
        }
        installs
            .into_iter()
            .filter(|(id, (_, at))| removals.get(id).is_none_or(|gone| at > gone))
            .map(|(_, (spec, _))| spec.clone())
            .collect()
    }

    /// Drop a package's finished entries, because somebody said LiNix has no relationship with
    /// it any more.
    ///
    /// `unmanage` means forget: it takes the package out of the registry, takes its line out of
    /// the manifest, and leaves the package installed. The log is the third record of the same
    /// relationship, and a `Completed` install left in it is exactly what
    /// [`completed_installs`](Self::completed_installs) reads as ownership the registry has
    /// merely lost — so the next sync would take the package back, find it declared nowhere,
    /// and remove software the user had asked LiNix only to stop watching. A command that
    /// silently undoes itself is the failure `unmanage` exists to avoid; this is the third
    /// place that had to be told.
    ///
    /// **Terminal entries only, and `Abandoned` is not one of them here.** An `InProgress` or
    /// `Abandoned` entry is the record that something on this machine is half-done, and
    /// forgetting a package is not a reason to lose it.
    ///
    /// `backend` narrows to one manager; `None` forgets the name under every manager, matching
    /// how the verb treats a bare name everywhere else.
    pub fn forget(&mut self, backend: Option<&str>, name: &str) -> Result<usize> {
        ...
        self.entries.retain(|_, entry| {
            ...
            let (b, n) = entry.action.identity();
            let named = n == name && backend.is_none_or(|want| want == b);
            !(finished && named)
        });
