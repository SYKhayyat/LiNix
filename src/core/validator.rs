...
impl Validator {
    ...
    pub fn validate_package_name(name: &str) -> Result<()> {
        if name.is_empty() { return Err(Error::Validation("Empty package name".into())); }
        if name.len() > 256 { return Err(Error::Validation("Name too long".into())); }

        // Bug 2: Explicitly block directory traversal
        if name.contains("..") || name.starts_with('/') || name.starts_with('\\') {
            return Err(Error::Validation(format!("Path traversal detected in name: {}", name)));
        }
        ...
        if SHELL_INJECTION_REGEX.is_match(name) {
            return Err(Error::Validation("Shell injection characters detected".into()));
        }
    ...
    pub fn validate_command(cmd: &str, args: &[&str]) -> Result<()> {
        ...
        for pattern in DESTRUCTIVE_PATTERNS {
            if full.contains(pattern) {
                return Err(Error::Validation(format!("Destructive command blocked: {}", full)));
            }
        }
        ...
        if cmd_path.is_absolute() {
            ...
            let canonical_cmd = cmd_path.canonicalize().unwrap_or_else(|_| cmd_path.to_path_buf());
            ...
            if !is_trusted {
                return Err(Error::Validation(format!("Untrusted binary origin: {}", cmd)));
            }
        }
    ...
    pub async fn validate_path(path: &Path) -> Result<PathBuf> {
        ...
        for forbidden in FORBIDDEN_PATHS {
            ...
            if canonical.starts_with(f_path) {
                warn!("Security Block: Attempted access to forbidden path: {:?}", canonical);
