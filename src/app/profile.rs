...
impl ProfileManager {
    ...
    async fn load_active(&self) -> Result<Vec<String>> {
        ...
        let body = tokio::fs::read_to_string(&path).await.map_err(Error::from)?;
        Ok(body
            .lines()
            .map(|l| l.trim())
    ...
    async fn materialize(&self, active: &[String]) -> Result<Vec<String>> {
        ...
        tokio::fs::create_dir_all(&self.config.groups_dir).await.ok();
...
///
/// Additions are collected first; subtractions (both `-pkg` and every `exclude`d profile's
/// packages) are applied last, so subtraction always wins. `visiting` guards against cyclic
/// `include`/`exclude` references — a profile already on the resolution stack contributes
/// nothing further. Unknown profile names resolve to the empty set (missing files are not an
/// error here; existence is validated at the `activate`/`switch` entry points).
fn compose(name: &str, defs: &HashMap<String, Vec<String>>, visiting: &mut HashSet<String>) -> Vec<String> {
