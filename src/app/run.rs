...
impl Runner {
    ...
    pub async fn run(&self, packages: &[String], command: &str, args: &[String]) -> Result<()> {
        info!("Runner: Provisioning environment for command '{}'...", command);
        ...
        for spec in &resolved_specs {
            ...
            let is_present = if let Some(queryable) = backend_caps.as_queryable() {
                ...
                debug!("Runner: Backend '{}' not queryable, assuming missing.", spec.backend);
                false
            };
        ...
        let status = if sandbox_requested {
            if can_sandbox {
                ...
                }).await.map_err(|e| Error::Other(format!("Sandbox thread failure: {}", e)))??
        ...
        if !status.success() {
            ...
            error!("Runner: Environment command failed with exit code {}.", code);
            return Err(Error::CommandFailed(format!("Sub-process exited with code {}", code)));
        }
    ...
    async fn execute_standard(&self, command: &str, args: &[String]) -> Result<std::process::ExitStatus> {
        ...
        let mut handle = child.spawn()
            .map_err(|e| Error::CommandFailed(format!("Failed to start binary {}: {}", command, e)))?;
