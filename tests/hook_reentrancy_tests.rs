//! A hook LiNix installs into a package manager must stand down when LiNix started that
...
//! zypper, apk, xbps, portage and eopkg invoke. It is not what pacman invokes — LiNix installs
...
//!
//! ```text
//! linix sync  (holds DataLock)
//!   └─ pacman -S foo                    ← spawned with LINIX_INSIDE set
//!        └─ PostTransaction hook
//!             └─ linix hook-record …    ← ignored LINIX_INSIDE, blocked for the full 120s
//! ```
//!
...
//! body LiNix writes into a manager's configuration and checks the subcommand it names is one
...
fn hook_invocations() -> Vec<Vec<String>> {
    let cmd = <linix::cli::Cli as clap::CommandFactory>::command();
    let mut out = Vec::new();
...
///
/// The lock is taken here rather than by a spawned `linix` on purpose. No verb holds it for its
...
struct HeldLock {
    dir: PathBuf,
    _lock: linix::core::datalock::DataLock,
}
...
/// Run one argv with `LINIX_INSIDE` set, against a data directory somebody else is holding.
///
...
fn run_bounded(dir: &std::path::Path, argv: &[String], bound: Duration) -> Option<(i32, Duration)> {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_linix"))
        .args(argv)
...
#[test]
fn every_hook_subcommand_stands_down_when_linix_started_the_manager() {
        ...
        match run_bounded(&held.dir, &argv, bound) {
            Some((code, took)) if took < bound => {
                assert_eq!(
                    code,
                    0,
                    "`linix {}` stood down but exited {code}; standing down is not a failure — \
    ...
    assert!(
        ...
        "{} hook subcommand(s) blocked on the data lock instead of standing down under \
         LINIX_INSIDE:\n  {}\n\nEach one is what a package manager LiNix is driving invokes \
...
/// what stops a hook body being written that calls `linix sync` — or a hook subcommand being
/// renamed to something that no longer matches — while the enumeration above still passes.
#[test]
fn every_hook_linix_installs_stands_down() {
    let src = std::fs::read_to_string(repo().join("src/app/pm_hooks.rs"))
        .expect("src/app/pm_hooks.rs is gone; re-derive this finding");
    let known: Vec<String> = <linix::cli::Cli as clap::CommandFactory>::command()
        .get_subcommands()
        .map(|s| s.get_name().to_string())
        .collect();

    // Every `{linix_bin} <word>` in a hook body: the binary placeholder is how each of those
    // format strings names LiNix, so this finds the invocation without knowing the shape of
    // the file the manager wants.
    let mut invoked: Vec<String> = Vec::new();
    for (_, rest) in src
        .match_indices("{linix_bin} ")
    ...
    assert!(
        !invoked.is_empty(),
        "no `{{linix_bin}} <subcommand>` was found in pm_hooks.rs; this extraction is looking \
         at the wrong file"
    );

    // **Not every `{linix_bin}` in this file is a hook body.** The shell-integration snippet
    // opens with a comment telling the reader how to source it — `eval "$(linix hooks
    ...
    assert!(
        ...
        "pm_hooks.rs writes {:?} into a package manager's hook file, and nothing there is \
         covered by `Commands::is_manager_hook`. A hook fired by a manager LiNix is driving \
