...
/// The capability tables, asked against the tools themselves.
///
/// **The gate above reads argvs, and that is the wrong instrument for a flag the runtime probe
/// withholds.** `tool_help::accepts_flag` asks the installed tool's help and drops a flag the
/// help does not document — a good thing to do and a terrible thing to do silently, because the
/// withheld flag is precisely the drifted one. So the argv the gate inspects has no flag in it
/// exactly when the table is wrong. Measured 2026-07-29 (N-4): with `--linix-bogus-flag-zzz`
/// planted in `VERIFIES_ITSELF` on a host where helm is `[READY]`, the gate was green in both
/// directions — the repo's signature defect occurring inside the check built to end it.
///
/// This asks the question an argv cannot carry: **does the installed tool document the flag this
/// table names?** A withheld flag is not a smaller problem than a rejected one. It is the same
/// drift, absorbed, and it leaves the user with `@unverified` on a line that cannot do anything
/// — `explain_verification` used to answer helm's refusal by advising the flag they had already
/// written.
///
/// Three outcomes and no silence: the tool documents it (ok), the tool does not (drift, and this
/// fails), or the tool could not be asked (a named skip, printed).
#[tokio::test]
async fn every_capability_flag_is_documented_by_the_tool_that_must_accept_it() {
    use dashmap::DashMap;
    use linix::core::executor::MockExecutor;
    use linix::core::{CommandExecutor, PackageSpec};
    use std::sync::Arc;
    ...
    for backend in registry.available() {
        ...
        // Drive the `@unverified` install to learn the subcommand chain the flag rides on —
        // `plugin install` for helm — because that is the help page that would document it.
        // Read from the argv LiNix builds rather than from the table, so a chain that changes
        // is followed rather than assumed. The chain survives even when the flag is withheld,
        ...
        let _ = installable
            ...
            .await;
        let calls = mock.get_calls().await;
        let Some(call) = calls.into_iter().nth(before) else {
            skipped.push(format!(
                "{name}: produced no argv for an `@unverified` install"
            ));
            continue;
        };
        ...
        let Some(program) = tokens.next() else {
            skipped.push(format!("{name}: empty argv"));
            continue;
        };
        let chain: Vec<String> = tokens
            .take_while(|t| !t.starts_with('-'))
            .map(String::from)
            .collect();

        if !on_path(program) {
            skipped.push(format!("{name}: `{program}` is not on this machine"));
            continue;
        }

        match linix::core::tool_help::accepts_flag(program, &chain, flag) {
            Some(true) => checked.push(format!("{name}: `{program} {} {flag}`", chain.join(" "))),
            Some(false) => drifted.push(format!(
                "`{name}`: the capability table says `@unverified` adds `{flag}`, and \
                 `{program} {} --help` does not document it. LiNix withholds the flag and says \
                 nothing the gate can see; the install is then subject to a verification the \
                 declaration asked to skip.",
                chain.join(" ")
            )),
            None => skipped.push(format!(
                "{name}: `{program} {} --help` could not be read",
                chain.join(" ")
            )),
        }
    }
    ...
    assert!(
        drifted.is_empty(),
        "a capability table names a flag the installed tool does not accept:\n  {}\n\n\
         This is drift, not a detail. The argv-reading half of this gate cannot see it — the \
         flag is dropped before an argv exists — which is why the question is asked of the tool \
         here instead.",
        drifted.join("\n  ")
    );

    // No silent pass: if nothing could be examined, say so rather than reporting success. The
    // count is not asserted upward because `VERIFIES_ITSELF` currently has one entry, and a
    // machine without helm legitimately checks zero.
    if checked.is_empty() {
        eprintln!(
            "  NOTE: no capability flag could be checked on this machine — no backend in a \
             capability table is installed here. This gate examined nothing; it is not evidence."
        );
    }
}
