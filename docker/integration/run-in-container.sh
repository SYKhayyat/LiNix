...
MEASURABLE=$((LIFECYCLES + EXCUSED + NOKERNEL))
if [ -n "$SMOKE" ]; then
    ...
    - coverage: $LIFECYCLES real lifecycle(s) on $HOST_CLASS, below the recorded $FLOOR"
        ...
        soft "real-lifecycle ratchet: $LIFECYCLES of $FLOOR on $HOST_CLASS, and $((EXCUSED + NOKERNEL)) backend(s) of $UNMEASURED unmeasurable are excused this run"
